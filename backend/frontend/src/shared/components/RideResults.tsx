...
// ✅ INTERFACE COMPATÍVEL com search.tsx
interface Ride {
  id: string;
  driverId?: string;
  fromAddress: string;
  ...
  type: string;
  driverName: string;
  driverRating: number | string; // ✅ CORRIGIDO: number | string em vez de apenas string
  description?: string;
  ...
  const { data: internalRides = [], isLoading } = useQuery<Ride[]>({
    ...
    queryFn: async () => {
      ...
      return backendRides.map((ride: any) => ({
        id: ride.id.toString(), // ✅ Garantir que seja string
        ...
        driverRating: ride.driverRating, // ✅ Pode ser number ou string
