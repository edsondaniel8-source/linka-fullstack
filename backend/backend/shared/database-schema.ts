...
// Bookings table (MOVED BEFORE PAYMENTS)
...
// CONSOLIDATED: Payments table (MOVED AFTER BOOKINGS)
...
export const events = pgTable("events", {
  ...
  socialMediaLinks: text("socialMediaLinks").array(),
...
export const partnershipProposals = pgTable("partnershipProposals", {
  ...
  targetRegions: text("targetRegions").array(),
  minimumDriverLevel: text("minimumDriverLevel").default("bronze"),
  requiredVehicleType: text("requiredVehicleType"),
  startDate: timestamp("startDate").notNull(),
  endDate: timestamp("endDate").notNull(),
  specificDates: text("specificDates").array(),
  timeSlots: text("timeSlots").array(),
  ...
  additionalBenefits: text("additionalBenefits").array(),
...
export const hotelRooms = pgTable("hotelRooms", {
  ...
  roomAmenities: text("roomAmenities").array(),
  isAvailable: boolean("isAvailable").default(true),
